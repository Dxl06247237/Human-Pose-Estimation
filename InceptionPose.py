...
class ConvMlp(nn.Module):
    ...
    def __init__(
            ...
            norm_layer=nn.BatchNorm2d, bias=True, drop=0.):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        bias = to_2tuple(bias)
...
class InceptionBlock(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.token_mixer = InceptionDWConv2d(in_channels=dim)
        self.norm = norm_layer(dim)
        self.mlp = mlp_layer(dim, int(mlp_ratio * dim), act_layer=act_layer)
        self.gamma = nn.Parameter(ls_init_value * torch.ones(dim)) if ls_init_value else None
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.MlpDWBN = MlpDWBN(dim, int(mlp_ratio * dim))
...
class Bottleneck4(nn.Module):
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.atten = EMA(channels=inplanes)
...
class PoseHigherResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.waspv2 = WASPv2(conv_type='SEPARABLE',inplanes=48,planes=48)
        self.up = Bottleneck4(inplanes=384,planes=192)
        self.up1 = Bottleneck4(inplanes=192,planes=96)
        self.up2 = Bottleneck4(inplanes=96, planes=48)

        self.iaff1 = iAFF1(channels=192)
        self.iaff2 = iAFF1(channels=96)
        self.iaff3 = iAFF1(channels=48)
